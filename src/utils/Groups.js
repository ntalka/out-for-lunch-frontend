...
export async function GetAllGroups() {
    const resJSON = getRequest("/get-groups-list", String(getUser()))
        .then((resJSON) => {
...
export async function joinGroup(groupId){
    await postRequest("/join-group/"+groupId, {}, String(getUser()))
        .then(() =>{
        });
...
export async function leaveGroup(groupId){
    await postRequest("/leave-group/"+groupId,{}, String(getUser()))
        .then(() =>{
        });
}

export function deleteGroup(groupId, joined){
    deleteRequest("/delete-group/"+groupId, String(getUser()))
        .then(()=>{
            if(joined){
...
export function joinRandomGroup(startTime, endTime){
    const body = {
        "endTime": endTime,
        "startTime": startTime}
    postRequest("/join-random-group", body, String(getUser()))
        .then(() =>{
        });
}
...
export async function createEatAtOffice(startTime){
    ...
    await postRequest("/eat-at-office", body, String(getUser()))
        .then(() =>{
    });
}
...
export async function createGroup(placeId, startTime){
    ...
    await postRequest("/create-custom-group", body, String(getUser()))
        .then(() =>{
        });
}
